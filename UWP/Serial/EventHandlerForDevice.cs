...
namespace SerialTemplate
{
    ...
    public class EventHandlerForDevice
    {
        ...
        public async Task<Boolean> OpenDeviceAsync(DeviceInformation deviceInfo, String deviceSelector)
        {
            ...
            if(deviceInfo.EnclosureLocation == null)
            {
                notificationStatus = NotifyType.ErrorMessage;
                notificationMessage = "Device " + deviceInfo.Name + " not available!";
            }
            else
            {
        ...
        /// not reopen the device automatically, so we need to explicitly open the device in the app (SerialConnectDisconnectPage).
        ///
